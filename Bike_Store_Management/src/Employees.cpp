...
void Employees::add_emp()
{
	...
	cin >> salary;
	stmt.str("");
	stmt << "Insert into employees (name,addr1,addr2,addr_city,addr_stat,phone_no,doj,salary) values ('" << name << "','" << addr_line1 << "','" << addr_line2 << "','" << addr_city << "','" << addr_state << "'," << phone << ",curdate()," << salary << ");";
	query = stmt.str();
	q = query.c_str();
	mysql_query(conn,q);
...
void Employees::assign_mgr_stat()
{
	...
	mysql_query(conn,q);
	cout << endl << endl << endl;
	cout << "Manager Status granted" << endl;
