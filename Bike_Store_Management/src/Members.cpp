...
void Members::add_mem()
{
	...
	cin >> addr_state;
	stmt.str("");
	stmt << "insert into members(name,addr1,addr2,addr_city,addr_stat,phone_no,beg_date,end_date) values ('" << name << "','" << addr_line1 << "','" << addr_line2 << "','" << addr_city << "','" << addr_state << "'," << phone << ",curdate(),Date_add(curdate(), INTERVAL 1 YEAR));";
	query = stmt.str();
	q = query.c_str();
	mysql_query(conn,q);
	stmt.str("");
	stmt << "Select id from members where phone_no = " << phone << ";";
	query = stmt.str();
	q = query.c_str();
	mysql_query(conn,q);
